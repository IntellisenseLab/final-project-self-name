...
class MockLeader(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('static_z', 1.5)         # metres ahead of robot
        self.declare_parameter('static_x', 0.0)         # metres to the side
        self.declare_parameter('move_speed', 0.1)        # metres per second (moving mode)
        self.declare_parameter('publish_rate', 20.0)     # Hz
 
        self.mode = self.get_parameter('mode').value
        self.static_z = self.get_parameter('static_z').value
        ...
        if self.mode == 'static':
            self.get_logger().info(f'Publishing static pose: z={self.static_z}m, x={self.static_x}m')
        else:
    ...
    def publish_pose(self):
        ...
        if self.mode == 'static':
            msg.pose.position.z = self.static_z
            ...
            self.current_z = self.static_z + (self.move_speed * self.elapsed)
            self.current_x = 0.3 * math.sin(0.5 * self.elapsed)   # gentle sine drift
